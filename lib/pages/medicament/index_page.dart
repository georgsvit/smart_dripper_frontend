...
class MedicamentsIndexPage extends StatefulWidget { 
  @override
  _MedicamentsIndexState createState() => _MedicamentsIndexState();
}

class _MedicamentsIndexState extends State<MedicamentsIndexPage> {
  ...
  void _deleteElement(String id) async {
    var result;
    try {
      await deleteMedicament(id).then((value) => result = value);
  ...
  Widget build(BuildContext context) {
    if (medicaments == null) {
      ...
      return Scaffold(
        appBar: AppBar(
          title: Text(AppLocalization.of(context).translate('medicaments_label')),                          
        ),
        body: Center(
          child: Column(
            ...
            children: <Widget>[
              Center(
                child: Text(
                  AppLocalization.of(context).translate('medicaments_list_label'),
                ...
                child: RaisedButton(
                  ...
                  onPressed: () => {
                    showDialog(context: context, builder: (BuildContext context) {                                  
                      return MedicamentFormPage(new MedicamentResponse("", "", "", "", "", 0, 0, null, null));
                            ...
                            children: [
                              Column(
                                ...
                                children: [
                                  ...
                                  Text(AppLocalization.of(context).translate('manufacturer_label') + ': ' + element.manufacturer.name),                                  
                                  Text(AppLocalization.of(context).translate('medical_protocol_label') + ': ' + element.medicalProtocol.title),                                  
                                  Text(AppLocalization.of(context).translate('amount_in_pack_label') + ': ' + element.amountInPack.toString()),
                                  Text(AppLocalization.of(context).translate('lack_label') + ': ' + element.lack.toString()),
                                ],
                              ),
                              IconButton(
                                ...
                                onPressed: () => {
                                  showDialog(context: context, builder: (BuildContext context) {                                  
                                    return MedicamentFormPage(element);
                                  }).then((value) => _getElements())                                  
                                },
